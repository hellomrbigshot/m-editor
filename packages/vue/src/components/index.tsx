...
export const MEditor = defineComponent({
  ...
  emits: ['on-change', 'on-mode-change', 'on-fullscreen-change', 'update:modelValue'],
  setup(props, { emit }) {
    const { showLineNum, placeholder, autoScroll, debounceRenderWait, debounceRender, theme } = toRefs(props)
    const iconLength = ref(TOOLBAR_CONFIG.length)
    ...
    const setHtml = (_value: string) => {
      if (debounceRender.value) {
        ...
        markedHtml.value = marked(_value)
      }
    }
    ...
    const debounceMarked = debounce((value: string) => {
      markedHtml.value = marked(value)
    }, debounceRenderWait.value)
    const handleScroll = autoScroll.value
      ? throttle(() => {
          // scrollEvent
          if (editWrapperRef.value && previewWrapperRef.value) {
            ...
            const previewWrapperRefMaxScrollTop =
              previewWrapperRef.value.scrollHeight - previewWrapperRef.value.clientHeight
            if (scrollType.value === 'edit') {
              previewWrapperRef.value.scrollTop =
                (editWrapperRef.value.scrollTop / editWrapperRefMaxScrollTop) * previewWrapperRefMaxScrollTop
            } else if (scrollType.value === 'preview') {
              editWrapperRef.value.scrollTop =
                (previewWrapperRef.value.scrollTop / previewWrapperRefMaxScrollTop) * editWrapperRefMaxScrollTop
            }
          }
        }, 200)
      : undefined
    ...
    const handleResize = () => {
      ...
      if (width > 780) {
        _length = TOOLBAR_CONFIG.length
      } else if (width > 680) {
        _length = TOOLBAR_CONFIG.length - 3
      } else if (width > 640) {
        _length = TOOLBAR_CONFIG.length - 6
      } else if (width > 500) {
        _length = 0
        mode.value = 'edit'
      }
    ...
    watchEffect(() => {
      ...
      // console.log('trigger')
      emit('on-change', { content: defaultValue.value, htmlContent: marked(defaultValue.value) })
      ...
      >
        ...
        <div class="editor-content">
          <div
            ...
            ref={editWrapperRef}
            onMouseenter={() => {
              scrollType.value = 'edit'
            }}
            onScroll={handleScroll}
          >
            <div class="editor-content-edit-block">
              {showLineNum.value ? <Column length={columnsLength.value} /> : ''}
          ...
          <div
            class={[
              ...
              mode.value === 'preview' && 'active',
              mode.value === 'edit' && 'inactive',
            ]}
...
MEditor.install = (app: App) => {
  app.component(MEditor.name as string, MEditor)
}
